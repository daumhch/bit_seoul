...
model = Sequential()
# Embedding(단어사전갯수, 아웃풋노드갯수, pad_x의 컬럼 갯수)
# model.add(Embedding(word_size, 10, input_length=5 )) # 동작
# model.add(Embedding(word_size, 10, input_length=6 )) # 동작하는데 warning
# model.add(Embedding(word_size, 10, input_length=3 )) # 동작하는데 warning
# model.add(Embedding(word_size, 10)) # 동작 -> Flatten 시 문제
# model.add(Embedding(word_size, 1)) # 동작
# model.add(Embedding(word_size+5, 1)) # 동작
# model.add(Embedding(word_size-5, 1)) # 문제
...
# 입력하지 않으면 (None, None, word_size)가 입력되어 Flatten이 되지 않는다
