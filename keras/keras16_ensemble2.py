...
# 3개의 데이터 까지 나눌 수 있다
x2_train, x2_rest, y2_train, y2_rest, y3_train, y3_rest = train_test_split(
    x2, y2, y3, train_size=0.6, test_size=0.4) # 6:4로 먼저 나누고
x2_test, x2_val, y2_test, y2_val, y3_test, y3_val = train_test_split(
    x2_rest, y2_rest, y3_rest, train_size=0.5, test_size=0.5) # 남은 4를 5:5로 나눔
...
print("y3_train.size", y3_train.size)
print("y3_val.size", y3_val.size)
print("y3_test.size", y3_test.size)
...
output3_0 = Dense(15, name='output3_0')(middle3)
...
model.fit([x1_train, x2_train], [y1_train, y2_train, y3_train], 
    ...
    validation_data=([x1_val, x2_val], [y1_val, y2_val, y3_val]), # 검증 데이터 사용하기
    verbose=0)

# 4. 평가, 예측
# 평가 데이터 넣어서 결과 보기
result = model.evaluate([x1_test,x2_test], [y1_test,y2_test, y3_test], batch_size=32) 
print("result : ", result)
...
y1_predict, y2_predict, y3_predict = model.predict([x1_test,x2_test]) # 평가 데이터 다시 넣어 예측값 만들기
# print("y_predict:\n", y_predict)
