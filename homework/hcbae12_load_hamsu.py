...
# 2.모델
model = load_model("./save/hcbae_model_save2.h5")
...
# 시퀀셜로도 가능함
model.layers.pop(0) # 로드된 모델의 input을 날린다
input1 = Input(shape=(4,1), name='input1')
dense = model(input1)
dense = Dense(180, activation='relu', name='d1')(dense)
dense = Dense(10, activation='relu', name='d2')(dense)
output1 = Dense(1, name='output1')(dense)
model = Model(inputs=input1, outputs=output1)
model.summary()
